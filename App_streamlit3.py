...
# ¡CORRECCIÓN CRÍTICA AQUÍ! Definimos claramente la columna "Km_Totales"
COLUMNS = ["Fecha", "Hora", "LotesIngresados", "Lotes_CamionA", "Lotes_CamionB", "Km_CamionA", "Km_CamionB", "Km_Totales"]
...
def get_history_data():
    ...
    try:
        ...
        # Ajustamos la validación para el nuevo número de columnas
...
def save_new_route_to_sheet(new_route_data):
    ...
    try:
        ...
        # Aseguramos que new_route_data ya contenga "Km_Totales"
        ...
        # Nota: Si el número de COLUMNS es incorrecto, fallará aquí.
        st.error(f"❌ Error al guardar datos en Google Sheets. Verifique que la Fila 1 tenga {len(COLUMNS)} columnas: {e}")
...
def calculate_statistics(df):
    ...
    # Se utiliza Km_Total que ya debería existir en el DataFrame si se grabó correctamente
    if 'Km_Totales' in df.columns:
        df['Km_Total'] = pd.to_numeric(df['Km_Totales'], errors='coerce').fillna(0)
    else:
...
if page == "Calcular Nueva Ruta":
        ...
        with st.spinner('Realizando cálculo óptimo y agrupando rutas'):
            try:
                ...
                if "error" in results:
                    ...
                    # 1. CALCULAR KM TOTALES COMBINADOS
                    km_a = res_a.get('distancia_km', 0)
                    km_b = res_b.get('distancia_km', 0)
                    km_totales = km_a + km_b
                    
                    # 2. Generar Enlaces Google Maps 
                    ...
                    # 3. Combinar GeoJSON para el enlace de comparación
                    ...
                    new_route = {
                        ...
                        "Km_CamionA": km_a,
                        "Km_CamionB": km_b,
                        "Km_Totales": km_totales # ¡CAMPO CALCULADO GUARDADO!
                    }
    ...
    if st.session_state.results:
        ...
        # Añadir el GeoJSON combinado al mapa (Trazas y Puntos)
    ...
    if df_historial.empty:
        ...
        if not daily_stats.empty:
            ...
            columns_to_show = {
                ...
                'Lotes_Ingresados_Total': 'Lotes Asignados',
