...
# =============================================================================
# 2. FUNCIONES AUXILIARES MATEMÁTICAS Y OPTIMIZACIÓN
# =============================================================================
...
def calculate_angle(coord):
    """Calcula el ángulo polar de una coordenada respecto al origen."""
    dx = coord[0] - COORDENADAS_ORIGEN[0]
    dy = coord[1] - COORDENADAS_ORIGEN[1]
    return atan2(dy, dx)

def get_ordered_route(points_names):
    """
    Ordena una lista de puntos usando Nearest Neighbor (Vecino más cercano).
    Retorna la lista ordenada y la distancia estimada total.
    """
    if not points_names:
        return [], 0
    
    unvisited = points_names.copy()
    ordered_path = []
    ...
    while unvisited:
        ...
        next_point = min(unvisited, key=lambda p: haversine(current_pos, COORDENADAS_LOTES[p]))
        dist = haversine(current_pos, COORDENADAS_LOTES[next_point])
        total_dist += dist
        current_pos = COORDENADAS_LOTES[next_point]
        ordered_path.append(next_point)
        unvisited.remove(next_point)
        
    # Sumar retorno al origen
...
def find_best_grouping_variable(all_lotes, min_group_size=1):
    """
    Reemplazo de itertools.combinations por Barrido Angular (Sweep Algorithm).
    Es mucho más rápido y escala para muchos lotes.
    """
    if not all_lotes:
        return [], [], 0

    # 1. Calcular ángulos y ordenar
    lotes_con_angulo = []
    for nombre in all_lotes:
        coord = COORDENADAS_LOTES[nombre]
        angle = calculate_angle(coord)
        lotes_con_angulo.append({'name': nombre, 'angle': angle})
    
    # Ordenar por ángulo
    lotes_con_angulo.sort(key=lambda x: x['angle'])
    sorted_names = [x['name'] for x in lotes_con_angulo]
    ...
    # 2. Buscar el mejor punto de corte en la lista circular ordenada
    # Si hay pocos lotes, asignamos todo a uno solo si es mejor
    min_size = 1 if n > 1 else 0
    
    for i in range(min_size, n - min_size + 1):
        group_a = sorted_names[:i]
        group_b = sorted_names[i:]
        
        _, dist_a = get_ordered_route(group_a)
        _, dist_b = get_ordered_route(group_b)
        
        if (dist_a + dist_b) < best_total_dist:
            best_total_dist = dist_a + dist_b
            best_group_a = group_a
            best_group_b = group_b
            
    # Caso borde: Si solo hay 1 lote, todo al grupo A
    if n == 1:
        best_group_a = sorted_names
        best_group_b = []
        _, best_total_dist = get_ordered_route(best_group_a)

    return best_group_a, best_group_b, round(best_total_dist / 1000, 2)
...
def solve_route_optimization(all_intermediate_stops):
    # 1. Agrupamiento Optimizado (Barrido Angular)
    group_a_names, group_b_names, min_internal_dist = find_best_grouping_variable(all_intermediate_stops)
    
    # 2. Ordenamiento Previo (Nearest Neighbor) para mejorar la respuesta de la API
    ordered_a_names, _ = get_ordered_route(group_a_names)
    ordered_b_names, _ = get_ordered_route(group_b_names)
    ...
    # --- RUTA A (AF820AB) ---
    if ordered_a_names:
        all_stops_coords_A = [COORDENADAS_ORIGEN] + [COORDENADAS_LOTES[name] for name in ordered_a_names] + [COORDENADAS_ORIGEN]
        response_A = make_api_request(all_stops_coords_A)
        
        if response_A:
            TOTAL_DISTANCE_KM_A = round(response_A['paths'][0]['distance'] / 1000, 2)
            # Como ya pre-ordenamos, usamos el orden devuelto por API (que debería ser similar o mejor)
            optimized_indices_A = response_A['paths'][0]['points_order']
            all_stops_names_A = ["Ingenio"] + ordered_a_names + ["Ingenio"]
            ...
            results["ruta_a"] = {
                ...
                "lotes_asignados": ordered_a_names,
        ...
        results["ruta_a"] = {"mensaje": "Sin asignación", "distancia_km": 0, "lotes_asignados": []}

    # RETARDO
    if ordered_a_names and ordered_b_names:
        time.sleep(1.5)

    # --- RUTA B (AE898TW) ---
    if ordered_b_names:
        all_stops_coords_B = [COORDENADAS_ORIGEN] + [COORDENADAS_LOTES[name] for name in ordered_b_names] + [COORDENADAS_ORIGEN]
        ...
        if response_B:
            ...
            all_stops_names_B = ["Ingenio"] + ordered_b_names + ["Ingenio"]
            ...
            results["ruta_b"] = {
                ...
                "lotes_asignados": ordered_b_names,
        ...
        results["ruta_b"] = {"mensaje": "Sin asignación", "distancia_km": 0, "lotes_asignados": []}
